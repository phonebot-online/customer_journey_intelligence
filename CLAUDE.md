...
- **AUD/GBP FX (procurement-corrected):** the right comparison is the procurement-month rate (sales lag procurement by ~30 days), not the sale-month rate. RBA monthly data: Feb 2025 1 GBP = A$2.0251 → Feb 2026 A$1.8939 = AUD **+6.48%** stronger (drives March 2026 sales). Mar 2025 A$2.0631 → Mar 2026 A$1.9279 = AUD **+6.55%** stronger (drives April 2026 sales). At mid 65% GBP-share of COGS this translates to **~A$15-19k/mo of GP tailwind**, NOT the A$25-30k/mo previously stated against April-spot rates. The wider 11% YoY April-vs-April comparison ignores the procurement-to-sale lag and overstates the COGS-relevant FX gift. Strip the procurement-corrected ~6.5% number from any YoY comparison. (See `.claude-memory-backup/project_fx_tailwind.md`)
...
**Verdict:** scaling back worked AS DEFENSE. Headline op profit only -$8k YoY (March 25→26) despite -$31k GP, thanks to $9k marketing + $14k AU staff + $4k PK staff savings. Per-marketing-$ efficiency improved +24.7% headline. ⚠️ **FX-stripped figures need recalc** — the original analysis used the 11% Apr-spot FX delta worth A$25-30k/mo. The procurement-corrected delta is ~6.5% worth A$15-19k/mo (see updated `project_fx_tailwind.md`), so underlying YoY GP decline is closer to -A$50k (March) than the previously claimed -A$56-61k. The "roughly flat YoY underlying" framing was too optimistic — there IS a real underlying decline that the cost cuts absorb but don't fully offset. May 2026 won't be a victory lap because EOFY pressure typically compresses GP% 2-4pp.

**Strategic recommendation:** semi-autopilot AU (cash engine), build UK as a strategic second engine because UK retail puts Phonebot on equal footing with the auctioneers (forces renegotiation or pushes procurement upstream). Don't try to scale AU paid marketing past current — diminishing returns kick in hard above $50k/mo. **Note on UAE:** Phonebot already operates a profitable mall store at **Burjuman Centre, Dubai** — this is an existing, operational retail location, not a hypothetical expansion target. There is an active Meta walk-in-traffic campaign for it on the AU master ad account (act_14359173). Do not frame UAE as a "side bet" or "future expansion" — it's a live channel.

**OOS leak finding (REVISED 2026-05-01 — original was overstated 3.7×):** original claim of ~A$8,250/30d (~A$99k/yr) was 30-day cumulative spend on SKUs that were currently-OOS at analysis time, but most of that spend happened DURING the in-stock period (pre-OOS, profitable). After splitting daily SKU spend into pre-OOS vs post-OOS halves: legitimate in-stock spend = A$15,465/30d; **true post-OOS leak = A$2,205/30d (~A$26.8k/yr)**, conservative lower bound A$21.5k/yr. The mechanism Jelena (Google Ads manager) described works: when GMC marks OOS, GMC→AW sync disables the listing, spend drops to zero. The leak is the sync-lag window — for ~75% of leaking SKUs the lag exceeds 7 days (sluggish or broken sync), and 47% of leak $ comes from SKUs with 22+ day sync delay. Worst current candidates: SKU 1425 (iPad Pro 10.5 256GB Grade A — A$109/8 days; AW correctly disabled Apr 22-23 per Jelena's chart, so this is just sync-lag), SKU 8307 (Galaxy Watch 8 Classic — A$102 over 11 active post-OOS days, sync lag 43 days, looks like genuine broken sync). See `analyses/2026-05-strategic-review/oos_leak/REVISED_post_oos_leak_analysis.csv` for the cleaned per-SKU breakdown. Action priority demoted from P0 to P2 — worth investigating sync failures on the 36 broken-sync SKUs, not worth a strategic crisis narrative.
