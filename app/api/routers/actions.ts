...
export const actionsRouter = router({
  ...
  priorityList: publicProcedure.query(async () => {
    return [
      ...
      {
        priority: 'P2' as const,
        title: 'Fix Facebook pixel + UTM tagging (fbclid bug)',
        why: '62% of FB-driven purchase events have a fbclid server bug. GA4 attributes nearly zero facebook/cpc sessions. Without this, FB measurement is structurally broken — even if holdout shows incremental, you cannot optimise.',
        action: 'Engineering: audit checkout flow for fbclid persistence. Add Brevo + Conversions API server-side fallback. Enable Meta Conversions API for purchase events.',
        eta: '2-4 weeks (engineering work)',
        impact: 'Unlocks accurate FB measurement; precondition for any aggressive scaling mode',
        risk: 'Zero (measurement infrastructure)',
        evidence: 'qa_checks/STEP3_QA_INDEX.md (FB row)',
      },
      {
  ...
  testQueue: publicProcedure.query(async () => {
    return {
      ...
      metaAdsFeatures: [
        { name: 'Meta Conversions API (server-side)', why: 'Fixes the fbclid bug. Pixel + CAPI together restore measurement. PRECONDITION for any FB scaling.', priority: 'P1' },
